...
/** Nominatim cerca parole intere, non prefissi: sotto le tre lettere non trova niente. */
 ...
 */
export default function CampoLuogo({ etichetta, segnaposto, aiuto, scelto, onScelto }: Props) {
  const [testo, setTesto] = useState(scelto?.citta ?? "");
  ...
  useEffect(() => {
    if (scelto) setTesto(scelto.citta ?? scelto.indirizzo);
  }, [scelto]);
  ...
  function scegli(luogo: Luogo) {
    onScelto(luogo);
    setTesto(luogo.citta ?? luogo.indirizzo);
    setAperto(false);
    setProposte([]);
  }
        ...
        >
          {proposte.map((luogo, indice) => (
            <li key={luogo.indirizzo} id={`${campo}-proposta-${indice}`} role="option" aria-selected={indice === evidenziata}>
              ...
              >
                <span className="block text-sm font-semibold">{luogo.citta ?? "Senza comune"}</span>
